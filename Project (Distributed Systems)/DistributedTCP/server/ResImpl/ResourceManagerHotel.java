...
public class ResourceManagerHotel{
	...
	public ResourceManagerHotel(){
		...
		System.out.println("RMHotel Online.");
		while(true){
			try{
				Socket connection = serverSock.accept();
				ResourceManagerHotel2 rmhd = new ResourceManagerHotel2(connection, lm, transHash, tidBase);
				rmhd.start();
			}
			catch(Exception e){
				e.printStackTrace();			
	...
	public static void main(String args[]) {
        ...
        ResourceManagerHotel rmc = new ResourceManagerHotel();
    }
    			...
    			while(true){
        			...
        			while (it.hasNext()) {
        			  ...
        			  if( tme - 1000 <= 0 ){
        				  ...
        				  ResourceManagerHotel2 mwd = new ResourceManagerHotel2(connection, lm, transHash, tidBase);
        				  mwd.abort(TID);
...
class ResourceManagerHotel2 extends Thread implements ResourceManager 
{
    ...
    public ResourceManagerHotel2( Socket s, LockManager lmm, HashMap<Integer, Stack<TransactionObj>> transHashH, HashMap<Integer, Integer> tidBaseB  ){
    	connection = s;
		this.lm = lmm;
			...
			try{
		        ...
		        switch(findChoice((String)arguments.elementAt(0))){
			        ...
			        case 4:  //new Room
			            ...
			            if( addRooms( Id, location, numRooms, price, TID ) )
			            	sendMessage("true");
			            else
			            	sendMessage("false");
			            }
			        ...
			        case 8: //delete Room
			            ...
			            if( deleteRooms( Id, location, TID ) )
			                sendMessage("true");
			            else
			                sendMessage("false");
			            }
			        ...
			        case 12: //querying a Room location
			            ...
			            int x = queryRooms( Id, location, TID );
			            String status;
			            if( x == -1 )
			            	status = "false";
			            else
			        ...
			        case 16: //querying a Room price
		            	...
		            	int x = queryRoomsPrice( Id, location, TID );
		            	String status;
		            	if( x == -1 )
		            	   status = "false";
		            	else
    ...
    protected int queryPrice(int id, String key, int pTID) {
        Trace.info("RM::queryRoomsPrice(" + id + ", " + key + ") called" );
        ReservableItem curObj = (ReservableItem) readData( id, key);
        int value = -1; 
        if ( curObj != null ){
        	try{
        ...
        Trace.info("RM::queryRoomsPrice(" + id + ", " + key + ") returns cost=$" + value );
        return value;        
    }
 ...
 // reserve an item
        ...
        if ( item == null ) {
            ...
            try{
        		if( lm.Lock( pTID, key, 1 ) ){
                	...
                	stack.push( tobjz );
                	///////
                	Hotel item2 = (Hotel)readData(id, key);
                	String s = item2.getKey();
                	Hotel item3 = new Hotel(s.substring(5), item2.getCount(), item2.getPrice());
    ...
    public boolean deleteFlight(int id, int flightNum, int pTID)
        throws RemoteException
    {
        return false;//deleteItem(id, Flight.getKey(flightNum));
    }
    ...
    public boolean addRooms(int id, String location, int count, int price, int pTID)
        throws RemoteException
    {
    	Trace.info("RM::addRooms(" + id + ", " + location + ", " + count + ", $" + price + ") called" );
        Hotel curObj = (Hotel) readData( id, Hotel.getKey(location) );
        if ( curObj == null ) {
            // Room location doesn't exist...add it
            ...
            try{
	            if( lm.Lock( pTID, newObj.getKey(), 1 ) ){
	            	...
	            	///////
	            	writeData( id, newObj.getKey(), newObj );
	                Trace.info("RM::addRooms(" + id + ") created new location " + location + ", count=" + count + ", price=$" + price );
	            }
            }
            catch(Exception e){
            	e.printStackTrace();
            ...
            // add count to existing Room location and update price...
        	try{
	            if( lm.Lock( pTID, curObj.getKey(), 1 ) ){
	            	...
	            	stack.push( tobjz );
	            	///////
	            	Hotel anObj = new Hotel( location, count, price );
	            	anObj.setCount( curObj.getCount() + count );
	            	if ( price == 0 )
	                    anObj.setPrice( curObj.getPrice() );
	                
	            	writeData( id, anObj.getKey(), anObj );
	            	Trace.info("RM::addRooms(" + id + ") modified existing location " + location + ", count=" + anObj.getCount() + ", price=$" + price );
	            }
            }
            catch(Exception e){
            	e.printStackTrace();
    ...
    public boolean deleteRooms(int id, String location, int pTID)
        throws RemoteException
    {
        return deleteItem(id, Hotel.getKey(location), pTID);
        
    }

    // Create a new car Hotelation or add cars to an existing location
    ...
    public boolean deleteCars(int id, String location, int pTID)
        throws RemoteException
    {
        return false;//deleteItem(id, Car.getKey(location));
    }
...
//    }
    ...
    public int queryRooms(int id, String location, int pTID)
        throws RemoteException
    {
        return queryNum(id, Hotel.getKey(location), pTID);
    }
    ...
    public int queryRoomsPrice(int id, String location, int pTID)
        throws RemoteException
    {
        return queryPrice(id, Hotel.getKey(location), pTID);
    }
    ...
    */
    ...
    public boolean reserveRoom(int id, int customerID, String location, int pTID)
        throws RemoteException
    {
        return reserveItem(id, customerID, Hotel.getKey(location), location, pTID);
    }
