...
public class ResourceManagerFlight{
	...
	public ResourceManagerFlight(){
		...
		System.out.println("RMFlight Online.");
		while(true){
			try{
				Socket connection = serverSock.accept();
				ResourceManagerFlight2 rmfd = new ResourceManagerFlight2(connection, lm, transHash, tidBase);
				rmfd.start();
			}
			catch(Exception e){
				e.printStackTrace();			
	...
	public static void main(String args[]) {
        ...
        ResourceManagerFlight rmf = new ResourceManagerFlight();
    }
    			...
    			while(true){
        			...
        			while (it.hasNext()) {
        			  ...
        			  if( tme - 1000 <= 0 ){
        				  ...
        				  ResourceManagerFlight2 mwd = new ResourceManagerFlight2(connection, lm, transHash, tidBase);
        				  mwd.abort(TID);
...
class ResourceManagerFlight2 extends Thread implements ResourceManager 
{
	...
	public ResourceManagerFlight2(Socket s, LockManager lmm, HashMap<Integer, Stack<TransactionObj>> transHashH, HashMap<Integer, Integer> tidBaseB  ){
		connection = s;
		this.lm = lmm;
	...
	public void run(){
		...
		try{
			...
			//System.out.println("From" + connection.getInetAddress().getHostName());
			...
			try{
		        ...
		        switch(findChoice((String)arguments.elementAt(0))){
			        ...
			        case 2:  //new flight
			            ...
			            if( addFlight( Id, flightNum, flightSeats, flightPrice, TID ) )
			            	sendMessage("true");
			            else
			            	sendMessage("false");
			            }
			        ...
			        case 6: //delete Flight
			            ...
			            if( deleteFlight( Id, flightNum, TID ) )
			                sendMessage("true");
			            else
			                sendMessage("false");
			            }
			        ...
			        case 10: //querying a flight
			            ...
			            try{
				            ...
				            //status = status + String.valueOf( queryFlight( Id, flightNum ) ); 
				            int x = queryFlight( Id, flightNum, TID );
				            String status;
				            if( x == -1 )
				            	status = "false";
				            else
			        ...
			        case 14: //querying a flight Price
			            ...
			            int x = queryFlightPrice( Id, flightNum, TID );
			            String status;
			            if( x == -1)
			            	status = "false";
			            else
			        ...
			        case 28: //Vote Request
			        	try{
				            ...
				            int crashCon = Integer.parseInt(crashString.substring(1,2));
				            //CP1 - RM Crash after receiving vote
				            if(crashCon == 1){
				            	System.out.println("CRASHING after receiving vote request.");
				            	System.exit(1);
				            }
				            //CP2 - no difference with previous 
				            if(crashCon == 2){
				            ...
				            sendMessage("true");
				            //CP3 - crash after sending answer
				            if(crashCon == 3){
    ...
    protected boolean deleteItem(int id, String key, int TID)
    {
        ...
        if ( curObj == null ) {
            ...
            if (curObj.getReserved()==0) {
            	try{
    	            if( lm.Lock( TID, curObj.getKey(), 1 ) ){
    	            	//Abort: writeData with curObj.getKey() and curObj
    	            	Stack<TransactionObj> stack = getHashPos( TID );
    ...
    protected boolean reserveItem(int id, int customerID, String key, String location, int pTID) {
        ...
        //if( lm.Lock(TID, strData, lockType))
        ...
        if ( item == null ) {
        	...
        	try{
        		if( lm.Lock( pTID, key, 1 ) ){
                	...
                	stack.push( tobjz );
                	///////
                	Flight item2 = (Flight)readData(id, key);
                	String s = item2.getKey();
                	Flight item3 = new Flight(Integer.parseInt(s.substring(7)), item2.getCount(), item2.getPrice());
        ...
        throws RemoteException, DeadlockException
    {
        ...
        if ( curObj == null ) {
            ...
            try{
	            if( lm.Lock( TID, newObj.getKey(), 1 ) ){
            ...
            try{
	            if( lm.Lock( TID, curObj.getKey(), 1 ) ){
	            	...
	            	stack.push( tobjz );
	            	///////
	            	Flight anObj = new Flight( flightNum, flightSeats, flightPrice );
	            	/*curObj.setCount( curObj.getCount() + flightSeats );
	            	if ( flightPrice > 0 ) {
	                    curObj.setPrice( flightPrice );
	                }*/
	            	anObj.setCount( curObj.getCount() + flightSeats );
	            	if ( flightPrice > 0 ) {
	                    anObj.setPrice( flightPrice );
	                }
	            	writeData( id, anObj.getKey(), anObj );
	            	Trace.info("RM::addFlight(" + id + ") modified existing flight " + flightNum + ", seats=" + anObj.getCount() + ", price=$" + flightPrice );
	            }
            }
            catch(Exception e){
            	e.printStackTrace();
    ...
    public int queryFlight(int id, int flightNum, int pTID)
        throws RemoteException, DeadlockException
    {
        return queryNum(id, Flight.getKey(flightNum), pTID);
    }
...
//    }
    ...
    public int queryFlightPrice(int id, int flightNum, int pTID )
        throws RemoteException, DeadlockException
    {
        return queryPrice(id, Flight.getKey(flightNum), pTID);
    }
    ...
    public int queryCarsPrice(int id, String location, int pTID)
        throws RemoteException, DeadlockException
    {
        return -1;//queryPrice(id, Car.getKey(location));
    }
    ...
    */
    ...
    public boolean reserveCar(int id, int customerID, String location, int TID)
        throws RemoteException, DeadlockException
    {
        return false;//reserveItem(id, customerID, Car.getKey(location), location);
    }
    ...
    public boolean reserveFlight(int id, int customerID, int flightNum, int pTID)
        throws RemoteException, DeadlockException, DeadlockException
    {
    	return reserveItem(id, customerID, Flight.getKey(flightNum), String.valueOf(flightNum), pTID);
    }
