...
public class LockManager
{
    ...
    public boolean Lock(int xid, String strData, int lockType) throws DeadlockException {
        ...
        try {
            ...
            BitSet bConvert = new BitSet(1); //POF - might be a problem here as it will try to convert 
            while (bConflict) {
            ...
            if (dataObj.getXId() == dataObj2.getXId()) {    
                ...
                if (dataObj.getLockType() == DataObj.READ) {
                    if (dataObj2.getLockType() == DataObj.WRITE) {
                        ...
                        System.out.println("Want READ, someone has WRITE A:" + dataObj.getXId() + "   B:" + dataObj2.getXId());
                        return true;
                    }
                    else {
                        // do nothing 
